...
// Definitions
#define PRUEBA_PIT (0)
#define PRUEBAS_RAY (0)
#define OUTSIDE_SPHERE (1)
...
void MadgwickAHRSupdate(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz) {
	...
	CTR += 1;
	#if PRUEBAS_RAY
		PRINTF("	roll	%f	pitch	%f	yaw	%f 	X	%f	Y	%f	Z	%f\r\n", roll*180/3.1416,pitch*180/3.1416,yaw*180/3.1416, X, Y, Z);
				/*if (pitch >= 0){
					sprintf(uart_mssg, "%0.2fU%0.2fU%0.2f\n\r",X, Y, Z);
				}else
				{
					sprintf(uart_mssg, "%0.2fU%0.2fU%0.2f\n\r",X, Y, Z);
				}
				if (CTR%15 == 0)
						UART_WriteBlocking(DEMO_UART, uart_mssg, sizeof(uart_mssg) - 1);*/
	#elif PRUEBA_PIT
